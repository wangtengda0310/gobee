...
| 禁止额外加严/改义 | 长文案没有的随机性、次数、持续时机等，短文案**不得擅自加上**（如长写「获得 1 张牌」、短写成「随机获得 1 张牌」→ 报）。长文案有的关键持续/失效条件，短文案若省略导致效果范围明显变宽，报 issue 或「需人工确认」 |
...
| 标点/格式导致不通 | 仅当严重影响理解时报；富文本标签（`<color>`、`<sprite>` 等）本身不报 |
...
与「通用规则 / 语义」相同：须跑 **SkillName↔Id**、**ShortSkillText**、**SkillText 文案质量与技能名指称**、**SettlementDes**、**Allusion**、**DesignThought**。
...
→ python "SkillUI_技能表现表/scripts/check_SkillUI_jinengbiaoxianbiao.py" "<路径>" --semantic-json
→ 按通用语义规则分析 SkillName↔Id、ShortSkillText、SkillText、SettlementDes、Allusion、DesignThought
