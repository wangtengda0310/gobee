# -*- coding: utf-8 -*-
"""校验 LimitTimeSkinRechargeReward_限时皮肤充值奖励配置表.xlsx：通用结构化规则（首轮自动生成）。"""
...
HEADER_ROW = 2
DATA_START_ROW = 4
SHEET_NAME = '限时皮肤充值|LimitSkinRecharge'
...
class Issue:
    ...
    def format_line(self) -> str:
        label = DISPLAY_FIELD or "Name"
        disp = self.display or "-"
        rid = self.row_id if self.row_id != "" else "-"
        return f"Id={rid} | {label}={disp} | {self.field} | {self.message}"
...
def validate_structural(path: Path) -> list[Issue]:
    ...
    for _, row in data.iterrows():
        ...
        if ID_TYPE == "int":
            ...
            if parsed is None:
                issues.append(Issue(raw_id, disp, "Id", f"Id 须为 int，实际: {raw_id}"))
                continue
            ...
            if ID_TYPE == "string" and not ID_STR_RE.match(text):
                issues.append(
                    Issue(text, disp, "Id", f"Id 须为标识符，实际: {raw_id}")
                )
            row_id = text
            seen.append(text)
        ...
        # soft format by type row
        for field in data.columns:
            if field in ("Id", "__types__"):
                continue
            value = row.get(field)
            if is_empty(value):
                continue
            ...
            if "Time" in field or field.endswith("Date"):
                ...
                if not DATE_RE.match(text):
                    # only flag if looks like datetime attempt
    ...
    for dup, n in counts.items():
        if n > 1:
            issues.append(Issue(dup, "", "Id", f"Id 重复出现 {n} 次"))
    return issues


def main() -> int:
    parser = argparse.ArgumentParser(description="校验 LimitTimeSkinRechargeReward_限时皮肤充值奖励配置表.xlsx")
    parser.add_argument("path", help="LimitTimeSkinRechargeReward_限时皮肤充值奖励配置表.xlsx 路径")
