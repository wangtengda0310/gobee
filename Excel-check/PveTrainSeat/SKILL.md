...
name: PveTrainSeat
description: |
  校验名将杀 PveTrainSeat.xlsx。
  当用户提到 PveTrainSeat.xlsx、PveTrainSeat配置检查时使用。
  公共流程见 Excel-check/SKILL.md；本文件仅含本表规则。
---

# PveTrainSeat — 配置表检查

校验 `PveTrainSeat.xlsx`（sheet：`座位配置表|PveTrainSeat`）。公共约定见 [Excel-check/SKILL.md](../SKILL.md)。

**表结构**：A（经典行表）
**主键列**：`Id`
**运行时输入**：用户提供 `PveTrainSeat.xlsx` 路径。默认不读外联表。
...
```bash
python "PveTrainSeat/scripts/check_PveTrainSeat.py" "<路径>"
python "PveTrainSeat/scripts/check_PveTrainSeat.py" "<路径>" --json
python "PveTrainSeat/scripts/check_PveTrainSeat.py" "<路径>" --semantic-json
```
