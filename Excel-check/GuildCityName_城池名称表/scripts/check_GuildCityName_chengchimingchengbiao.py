# -*- coding: utf-8 -*-
"""校验 GuildCityName_城池名称表.xlsx（表结构 A，主键 name）。"""
...
HEADER_ROW = 0
DATA_START_ROW = 1
SHEET_NAME = '城池名|GlobalCityNameConfig'
PK_FIELD = 'name'
L1_FIELDS = ['description']
...
def load_rows(path: Path) -> pd.DataFrame:
    ...
    types = ([None] * len(headers)) if TYPE_ROW is None else raw.iloc[TYPE_ROW].tolist()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="校验 GuildCityName_城池名称表.xlsx")
