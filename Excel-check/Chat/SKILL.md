...
description: |
  校验名将杀 Chat.xlsx。
  当用户提到 Chat.xlsx、Chat配置检查时使用。
  公共流程见 Excel-check/SKILL.md；本文件仅含本表规则。
---

# Chat — 配置表检查

校验 `Chat.xlsx`（sheet：`聊天表|Chat`）。公共约定见 [Excel-check/SKILL.md](../SKILL.md)。

**表结构**：A（经典行表）
**主键列**：`Id`
**运行时输入**：用户提供 `Chat.xlsx` 路径。默认不读外联表。
...
```bash
python "Chat/scripts/check_Chat.py" "<路径>"
python "Chat/scripts/check_Chat.py" "<路径>" --json
python "Chat/scripts/check_Chat.py" "<路径>" --semantic-json
```
...
主要字段：`Id`, `Name`, `Weight`, `Level`, `TimeInterval`
