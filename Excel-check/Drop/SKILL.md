...
description: |
  校验名将杀 Drop.xlsx。
  当用户提到 Drop.xlsx、Drop配置检查时使用。
  公共流程见 Excel-check/SKILL.md；本文件仅含本表规则。
---

# Drop — 配置表检查

校验 `Drop.xlsx`（sheet：`掉落规则表|DropRule`）。公共约定见 [Excel-check/SKILL.md](../SKILL.md)。

**表结构**：A（经典行表）
**主键列**：`Id`
**运行时输入**：用户提供 `Drop.xlsx` 路径。默认不读外联表。
...
```bash
python "Drop/scripts/check_Drop.py" "<路径>"
python "Drop/scripts/check_Drop.py" "<路径>" --json
python "Drop/scripts/check_Drop.py" "<路径>" --semantic-json
```
...
主要字段：`Id`, `Name`, `Count`, `DropGroup`, `EnsureSmall`, `EnsureSmallGroup`, `EnsureBig`, `EnsureBigGroup`, `EnsureItemCount`, `EnsureItemID`, `ItemCheckExist`, `KeepEnsureCount`
