...
python "RougeEndlessEventRestNode_千里单骑无尽事件及休息节点表/scripts/check_RougeEndlessEventRestNode_qianlidanqiwujinshijianjixiuxijiedianbiao.py" "<路径>" --semantic-json
```

Issue：`EventNodeId=<EventNodeId> | BattleEndNodes=… | <字段> | <说明>`
...
| S9 | 部分 | 类型行为数组等时 |
| S11 | 部分 | 成对时间字段（若存在） |
| L1 | 是 | `BattleEndNodes` / `EventSectionTitle1` / `EventSectionDesc1` / `EventSectionTitle2` / `EventSectionDesc2` / `EventSectionTitle3` / `EventSectionDesc3` / `EventNodeName` / `EventNodeDes` |
...
- **L1 文案质量**：对非空 `BattleEndNodes`、`EventSectionTitle1`、`EventSectionDesc1`、`EventSectionTitle2`、`EventSectionDesc2`、`EventSectionTitle3`、`EventSectionDesc3`、`EventNodeName`、`EventNodeDes` 检查错字、漏字、病句、标点不规范（剥富文本标签后审）。
...
<!-- sync: 须含 L1：BattleEndNodes, EventSectionTitle1, EventSectionDesc1, EventSectionTitle2, EventSectionDesc2, EventSectionTitle3, EventSectionDesc3, EventNodeName, EventNodeDes -->
