# -*- coding: utf-8 -*-
"""校验 RougeEndlessValidator_千里单骑无尽条件验证表.xlsx：通用结构化规则（表结构 A，主键 ValID）。"""
...
HEADER_ROW = 2
DATA_START_ROW = 4
SHEET_NAME = '无尽条件表|RougeEndlessValidator'
PK_FIELD = 'ValID'
...
def main() -> int:
    parser = argparse.ArgumentParser(description="校验 RougeEndlessValidator_千里单骑无尽条件验证表.xlsx")
    parser.add_argument("path", help="RougeEndlessValidator_千里单骑无尽条件验证表.xlsx 路径")
