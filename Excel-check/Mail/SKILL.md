...
| L1 | 是 | `Title` / `Sender` / `Body` 文案质量（错字漏字病句标点） |

读表约定：连续空 3 行截断；第 2、3 行皆空列丢弃；`#` 分区行跳过。
...
- **L1 文案质量**：对非空 `Title`、`Sender`、`Body`（Body 先剥富文本标签再审）检查错字、漏字、病句、标点不规范。  
- 人物匹配等玩法语义见下方 **独有语义**（不属于公共 L*）。
