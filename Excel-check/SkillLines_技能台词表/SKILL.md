...
**表结构**：A（经典行表）
**主键列**：`Id`
**运行时输入**：用户提供 `SkillLines_技能台词表.xlsx` 路径。默认不读外联表。
...
- **Id**：int，不重复（S1）
- **SkinId**：可空；有值须为 int（S12）
- **SkillId**：可空；有值须为 ESkillId（标识串或 int）（S12）
- **SkillFirstLine / SkillSecondLine / SkillThirdLine / SkillForthLine**：可空；有值须为 int 或逗号分隔 int 列表（S12）
...
<!-- sync: S12 类型行；L1 不适用（台词列为 int[] 音效 ID） -->
