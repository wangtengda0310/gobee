...
name: SkillCardSort
description: |
  校验名将杀 SkillCardSort.xlsx。
  当用户提到 SkillCardSort.xlsx、SkillCardSort配置检查时使用。
  公共流程见 Excel-check/SKILL.md；本文件仅含本表规则。
---

# SkillCardSort — 配置表检查

校验 `SkillCardSort.xlsx`（sheet：`卡牌排序|SkillCardSort`）。公共约定见 [Excel-check/SKILL.md](../SKILL.md)。

**表结构**：A（经典行表）
**主键列**：`Id`
**运行时输入**：用户提供 `SkillCardSort.xlsx` 路径。默认不读外联表。
...
```bash
python "SkillCardSort/scripts/check_SkillCardSort.py" "<路径>"
python "SkillCardSort/scripts/check_SkillCardSort.py" "<路径>" --json
python "SkillCardSort/scripts/check_SkillCardSort.py" "<路径>" --semantic-json
```
