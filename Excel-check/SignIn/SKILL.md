...
description: |
  校验名将杀 SignIn.xlsx。
  当用户提到 SignIn.xlsx、SignIn配置检查时使用。
  公共流程见 Excel-check/SKILL.md；本文件仅含本表规则。
---

# SignIn — 配置表检查

校验 `SignIn.xlsx`（sheet：`签到表|SignIn`）。公共约定见 [Excel-check/SKILL.md](../SKILL.md)。

**表结构**：A（经典行表）
**主键列**：`Id`
**运行时输入**：用户提供 `SignIn.xlsx` 路径。默认不读外联表。
...
```bash
python "SignIn/scripts/check_SignIn.py" "<路径>"
python "SignIn/scripts/check_SignIn.py" "<路径>" --json
python "SignIn/scripts/check_SignIn.py" "<路径>" --semantic-json
```

Issue：`Id=<Id> | Title=… | <字段> | <说明>`
...
主要字段：`Id`, `UpperPoint`, `LowerPoint`, `Reward`, `Title`, `Content`
...
- **L1 文案质量**：对非空 `Title`、`Content` 检查错字、漏字、病句、标点不规范（剥富文本标签后审）。
