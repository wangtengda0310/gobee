...
description: |
  校验名将杀 Fx配置表.xlsx。
  当用户提到 Fx配置表.xlsx、Fx配置表配置检查时使用。
  公共流程见 Excel-check/SKILL.md；本文件仅含本表规则。
---

# Fx配置表 — 配置表检查

校验 `Fx配置表.xlsx`（sheet：`Fx表|Fx`）。公共约定见 [Excel-check/SKILL.md](../SKILL.md)。

**表结构**：A（经典行表）
**主键列**：`Id`
**运行时输入**：用户提供 `Fx配置表.xlsx` 路径。默认不读外联表。
...
```bash
python "Fx配置表/scripts/check_Fxpeizhibiao.py" "<路径>"
python "Fx配置表/scripts/check_Fxpeizhibiao.py" "<路径>" --json
python "Fx配置表/scripts/check_Fxpeizhibiao.py" "<路径>" --semantic-json
```

Issue：`Id=<Id> | Note=… | <字段> | <说明>`
...
主要字段：`Id`, `Note`, `Layer`, `AssetPath`, `NeedPreloaded`, `AudioId`, `AudioDelay`, `IsFollowStop`, `FxDelay`, `FxAudioInfo`
...
- **L1 文案质量**：对非空 `Note` 检查错字、漏字、病句、标点不规范（剥富文本标签后审）。
