...
name: Validator
description: |
  校验名将杀 Validator.xlsx。
  当用户提到 Validator.xlsx、Validator配置检查时使用。
  公共流程见 Excel-check/SKILL.md；本文件仅含本表规则。
---

# Validator — 配置表检查

校验 `Validator.xlsx`（sheet：`条件判断器|Validator`）。公共约定见 [Excel-check/SKILL.md](../SKILL.md)。

**运行时输入**：用户提供 `Validator.xlsx` 路径。默认不读外联表。
...
```bash
python "Validator/scripts/check_Validator.py" "<路径>"
python "Validator/scripts/check_Validator.py" "<路径>" --json
```
...
主要字段：`Id`, `StrID`, `ValidatorType`, `ValidatorParam`, `LogicSymbol`, `NextValidator`, `IsInvert`

#### 字段细则

- **Id**：int，不重复（S1）
- 除 `Id` 外其余字段默认**可空**；有值时按类型行做格式校验（S9 等，见脚本）
- **IsInvert** 等：可空；有值须 bool 语义（S4）
