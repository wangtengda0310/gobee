...
description: |
  校验名将杀 Achieve.xlsx。
  当用户提到 Achieve.xlsx、Achieve配置检查时使用。
  公共流程见 Excel-check/SKILL.md；本文件仅含本表规则。
---

# Achieve — 配置表检查

校验 `Achieve.xlsx`（sheet：`成就表|Achieve`）。公共约定见 [Excel-check/SKILL.md](../SKILL.md)。

**表结构**：A（经典行表）
**主键列**：`Id`
**运行时输入**：用户提供 `Achieve.xlsx` 路径。默认不读外联表。
...
```bash
python "Achieve/scripts/check_Achieve.py" "<路径>"
python "Achieve/scripts/check_Achieve.py" "<路径>" --json
python "Achieve/scripts/check_Achieve.py" "<路径>" --semantic-json
```
...
| S9 | 部分 | 类型行为数组等时 |
| S11 | 部分 | 成对时间字段（若存在） |
| L1 | 是 | `Name` / `Des` / `Condition` |

主要字段：`Id`, `Name`, `IsHide`, `CompleteCondId`, `Reward`, `Des`, `Condition`, `HeroItemId`, `OpenDate`
...
- **L1 文案质量**：对非空 `Name`、`Des`、`Condition` 检查错字、漏字、病句、标点不规范（剥富文本标签后审）。
