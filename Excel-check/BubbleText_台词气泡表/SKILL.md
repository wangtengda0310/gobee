---
name: BubbleText_台词气泡表
description: |
  校验名将杀 BubbleText_台词气泡表.xlsx。
  当用户提到 BubbleText_台词气泡表.xlsx、BubbleText_台词气泡表配置检查时使用。
  公共流程见 Excel-check/SKILL.md；本文件仅含本表规则。
---

# BubbleText_台词气泡表 — 配置表检查

校验 `BubbleText_台词气泡表.xlsx`（sheet：`台词气泡表|BubbleText`）。公共约定见 [Excel-check/SKILL.md](../SKILL.md)。

**表结构**：A（经典行表）
**主键列**：`Id`
**运行时输入**：用户提供 `BubbleText_台词气泡表.xlsx` 路径。默认不读外联表。
...
```bash
python "BubbleText_台词气泡表/scripts/check_BubbleText_taiciqipaobiao.py" "<路径>"
python "BubbleText_台词气泡表/scripts/check_BubbleText_taiciqipaobiao.py" "<路径>" --json
python "BubbleText_台词气泡表/scripts/check_BubbleText_taiciqipaobiao.py" "<路径>" --semantic-json
```
...
主要字段：`Id`, `BubbleType`, `SubType`, `Des`, `AudioId`
