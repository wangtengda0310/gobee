...
description: |
  校验名将杀 Team.xlsx。
  当用户提到 Team.xlsx、Team配置检查时使用。
  公共流程见 Excel-check/SKILL.md；本文件仅含本表规则。
---

# Team — 配置表检查

校验 `Team.xlsx`（sheet：`队伍|Team`）。公共约定见 [Excel-check/SKILL.md](../SKILL.md)。

**运行时输入**：用户提供 `Team.xlsx` 路径。默认不读外联表。
...
```bash
python "Team/scripts/check_Team.py" "<路径>"
python "Team/scripts/check_Team.py" "<路径>" --json
```
...
主要字段：`Id`, `Desc`, `NumLimit`, `ModeId`, `CanChangePool`, `SupportMatchAI`, `Cost`, `IsRecordLastMatcher`

#### 字段细则

- **Id**：int，不重复（S1）
- 除 `Id` 外其余字段默认**可空**；有值时按类型行做格式校验（S9 等，见脚本）
- **CanChangePool / IsRecordLastMatcher** 等：可空；有值须 bool 语义（S4）
