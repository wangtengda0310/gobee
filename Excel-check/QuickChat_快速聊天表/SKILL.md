---
name: QuickChat_快速聊天表
description: |
  校验名将杀 QuickChat_快速聊天表.xlsx。
  当用户提到 QuickChat_快速聊天表.xlsx、QuickChat_快速聊天表配置检查时使用。
  公共流程见 Excel-check/SKILL.md；本文件仅含本表规则。
---

# QuickChat_快速聊天表 — 配置表检查

校验 `QuickChat_快速聊天表.xlsx`（sheet：`快速聊天表|QuickChat`）。公共约定见 [Excel-check/SKILL.md](../SKILL.md)。

**表结构**：A（经典行表）
**主键列**：`Id`
**运行时输入**：用户提供 `QuickChat_快速聊天表.xlsx` 路径。默认不读外联表。
...
```bash
python "QuickChat_快速聊天表/scripts/check_QuickChat_kuaisuliaotianbiao.py" "<路径>"
python "QuickChat_快速聊天表/scripts/check_QuickChat_kuaisuliaotianbiao.py" "<路径>" --json
python "QuickChat_快速聊天表/scripts/check_QuickChat_kuaisuliaotianbiao.py" "<路径>" --semantic-json
```
