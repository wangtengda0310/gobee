...
description: |
  校验名将杀 UI文本配置表.xlsx。
  当用户提到 UI文本配置表.xlsx、UI文本配置表配置检查时使用。
  公共流程见 Excel-check/SKILL.md；本文件仅含本表规则。
---

# UI文本配置表 — 配置表检查

校验 `UI文本配置表.xlsx`（sheet：`UI文本|UIText`）。公共约定见 [Excel-check/SKILL.md](../SKILL.md)。

**表结构**：A（经典行表）
**主键列**：`key`
**运行时输入**：用户提供 `UI文本配置表.xlsx` 路径。默认不读外联表。
...
```bash
python "UI文本配置表/scripts/check_UIwenbenpeizhibiao.py" "<路径>"
python "UI文本配置表/scripts/check_UIwenbenpeizhibiao.py" "<路径>" --json
python "UI文本配置表/scripts/check_UIwenbenpeizhibiao.py" "<路径>" --semantic-json
```

Issue：`key=<key> | CnText=… | <字段> | <说明>`
...
- **L1 文案质量**：对非空 `CnText` 检查错字、漏字、病句、标点不规范（剥富文本标签后审）。
