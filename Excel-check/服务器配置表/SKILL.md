...
description: |
  校验名将杀 服务器配置表.xlsx。
  当用户提到 服务器配置表.xlsx、服务器配置表配置检查时使用。
  公共流程见 Excel-check/SKILL.md；本文件仅含本表规则。
---

# 服务器配置表 — 配置表检查

校验 `服务器配置表.xlsx`（sheet：`服务器配置表|Server`）。公共约定见 [Excel-check/SKILL.md](../SKILL.md)。

**表结构**：A（经典行表）
**主键列**：`Id`
**运行时输入**：用户提供 `服务器配置表.xlsx` 路径。默认不读外联表。
...
```bash
python "服务器配置表/scripts/check_fuwuqipeizhibiao.py" "<路径>"
python "服务器配置表/scripts/check_fuwuqipeizhibiao.py" "<路径>" --json
python "服务器配置表/scripts/check_fuwuqipeizhibiao.py" "<路径>" --semantic-json
```

Issue：`Id=<Id> | ServelName=… | <字段> | <说明>`
...
主要字段：`Id`, `ServelName`, `IsSave`, `IpPort`, `IpPortHeroPoint`, `KeepAlive`, `ServerZoneId`
...
- **L1 文案质量**：对非空 `ServelName` 检查错字、漏字、病句、标点不规范（剥富文本标签后审）。
