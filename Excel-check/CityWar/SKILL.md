...
description: |
  校验名将杀 CityWar.xlsx。
  当用户提到 CityWar.xlsx、CityWar配置检查时使用。
  公共流程见 Excel-check/SKILL.md；本文件仅含本表规则。
---

# CityWar — 配置表检查

校验 `CityWar.xlsx`（sheet：`兵临城下|CityWar`）。公共约定见 [Excel-check/SKILL.md](../SKILL.md)。

**表结构**：A（经典行表）
**主键列**：`Id`
**运行时输入**：用户提供 `CityWar.xlsx` 路径。默认不读外联表。
...
```bash
python "CityWar/scripts/check_CityWar.py" "<路径>"
python "CityWar/scripts/check_CityWar.py" "<路径>" --json
python "CityWar/scripts/check_CityWar.py" "<路径>" --semantic-json
```
...
主要字段：`Id`, `Name`, `CityImage`, `CityBgImage`, `FraySkill`, `CitySkill`, `LianJunSkill`, `CardPileID`, `HeroRank`, `CityDes`
...
- **L1 文案质量**：对非空 `Name`、`CityDes` 检查错字、漏字、病句、标点不规范（剥富文本标签后审）。
