...
type BeforeImportRecords func(records []map[string][]byte)
...
// 导入数据到指定表
func Import(db *sql.DB, database string, table string, records []map[string][]byte) {
	...
	// 2. 获取主键信息
	pkColumns, err := PetPrimaryKeyColumns(db, database, table)
	if err != nil {
...
func recordExists(tx *sql.Tx, table string, pkColumns []string, record map[string][]byte) (bool, error) {
	...
	for _, col := range pkColumns {
		if col == "uid" {
			b := record[col]
			uidVal := binary.LittleEndian.Uint64(b) // 或根据实际类型转换
