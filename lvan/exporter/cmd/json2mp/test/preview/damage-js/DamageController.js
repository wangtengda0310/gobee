function exec_command(cmdName, context, entity, ...args){
    console.log("【客户端伤害公式日志】 ", args);
}
window.game = (function (exports) {
    'use strict';
    class DamageController {
        ...
        get_skill_damage_rate(context) {
          ...
          get_hit_count(cur_attack_entity) {
            return cur_attack_entity ? cur_attack_entity.getHitCount() : 0;
          }
          ...
          damage_hit(context, rateId, attack_entity, target_entity, user) {
            let value = this["get_hit_rate_" + rateId](context, attack_entity, target_entity, user);
          ...
          get_buff_lefttime(context, entity, buffid){
            if(entity)
                return entity.getBuffLeftTime(buffid);
            return 0;
          }

          get_buff_count(context, entity, buffid){
              if(entity)
                  return entity.getBuffCount(buffid);
              return 0;
          }

        get_buff_interval(context, entity, buffid){
            if(entity)
...
damage_10003(context, init_damage, attack_entity, current_entity, target_entity, user) {
  ...
  return   zl001
    ...
    class BridgeUtil {
        /**获取伤害公式函数 */
        static getDamageFormulaById(id, context, init_damage, attack_entity, cur_attack_entity, target_entity, user) {
          return this._controller[BridgeUtil.prefix + id].call(this._controller, context, init_damage, attack_entity,cur_attack_entity, target_entity, user);
        }
      }
