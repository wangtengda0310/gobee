...
func tryParseXML(filepathstr string) {
	...
	for _, table := range module.Tables {
		...
		if module.Name == "" {
			msgpackfilename = fmt.Sprintf("%s/%smanifest.bytes", os.Args[2], name)
		} else {
			msgpackfilename = fmt.Sprintf("%s/%s_%smanifest.bytes", os.Args[2], strings.ToLower(module.Name), name)
		}
		...
		if module.Name == "" {
			manifestfilename = fmt.Sprintf("%s/%s.bytes", os.Args[2], name)
		} else {
			manifestfilename = fmt.Sprintf("%s/%s_%s.bytes", os.Args[2], strings.ToLower(module.Name), name)
		}
