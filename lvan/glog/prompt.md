...
- struct标签：主要定义日志的名字、版本、描述等属性
- entry标签：主要定义日志的名字、类型等属性,分base/ext，详见下文

## 四、生成规则
### 1. 日志函数
- 每个struct标签生成一个日志函数以及相关的参数结构体和辅助函数和单元测试，有两种方案:name属性为函数名
  - name属性为函数名，生成函数名为<name>Log
  - name属性为报名，生成函数名为Glog(<params>)
  初次生成代码需要提示用户选择方案，后续生成保持一致
  如已有代码不符合规范，需引导用户修改提示词或调整代码与方案保持一致
- 其他属性作为注释，每行一条，但desc属性对应的注释在第一行
- 函数参数为struct标签下的entry，按order顺序排列，详见下文

### 2. 参数结构体
- struct标签生成参数结构体，命名为函数名+Param
- entry标签生成参数结构体字段，命名为驼峰化的name属性
- base类型entry生成公共BaseParam结构体，所有参数结构体嵌入
  生成的BaseParam结构体存放目录需要根据前面日志行数目录结构有所区别，提示并协助用户修改此处提示词确定方案
- ext类型entry生成各自结构体字段，type映射见下
...
### 4. formatter函数
- 每个日志函数生成一个不导出的formatter函数，参数为结构体字段，顺序与order一致
- formatter函数内部用stringbuffer拼接参数，'|'分隔，调用write写入log.txt
- formatter函数命名规则：函数名+Helper，私有
- **formatter函数对write的调用采用依赖注入，write为参数传入，便于mock和性能测试。**
...
- **所有生成的类型、方法、formatter函数名需全局唯一，避免命名冲突。建议每个日志类型单独生成一个包/目录。**
...
### 1. 代码生成主流程模板化
- 使用`text/template`包替换原有字符串拼接，所有结构体、日志函数、formatter函数等均用模板渲染。
...
- 结构体、formatter函数、日志函数参数顺序严格按order排序。
- 注释完整，便于维护。
...
- **每个struct生成的日志函数、formatter函数、参数结构体、单元测试（_test.go）和性能测试（_bench_test.go）均应拆分为独立文件，不再集中在同一个文件。**
- **性能测试文件命名为<struct>_bench_test.go，例如online_bench_test.go、chat_bench_test.go，单元测试文件为<struct>_test.go。**
- **每个struct相关的go文件、测试文件建议放在独立目录或以文件名区分，结构如下：**
  ```
  output/
    online/
      online.go
      online_test.go
      online_bench_test.go
    chat/
      chat.go
      chat_test.go
      chat_bench_test.go
  ...
  output/
    ...
    base_param.go
  ```
- **每个struct生成的go文件仅包含该struct相关的内容，测试和benchmark只测试对应struct的日志逻辑，避免交叉。**
...
// onlineLogFormatterer formatter函数
