...
// 布局层:App.vue 入口按 UA(Android)挂 .is-mobile。移动端 inline 模式改为换行 + 输入框自适应,
// 避免 input-width 固定(如 280px)×2 溢出窄屏。PC 不变。
...
<template>
  <!-- inline模式：使用 n-space 水平排列。移动端(.is-mobile)改为换行 + 输入框自适应,PC :wrap=false 不变 -->
  <n-space v-if="layout === 'inline'" align="center" :size="10" :wrap="isMobile">
    <n-input-group :style="isMobile ? { flex: '1 1 100%' } : undefined">
      ...
      <n-input
        ...
        :size="size"
        :style="isMobile ? { flex: '1 1 0', width: 'auto', minWidth: '0' } : { width: inputWidth }"
