...
import {refreshSkillDescConfig} from "../config/SkillUI";
import {flushLogCacheNow, initLogStatistic, resetLogCache} from "./RobotTestLog";
import {selectCase} from "./use-case-selection";
...
const execCaseApi = (message: MessageApiInjection, cases: (TreeOption & ExtraCaseTreeOption)[], dir: string) => {
    ...
    if (cases.length > 0)
        JsonCaseService.RunRobotTest(ServerAddr.value, ServerPort.value.toString(), RobotPrefix.value, Desc.value, FeiShuGuid.value, LoginTime.value, [], [],
            null, temp_file, RoomOpTime.value, FeiShuNtf.value, DebugLevel.value, DebugLog.value, Concurrency.value, 0
        ).then(res => {
            flushLogCacheNow()
            message.info("用例运行结束")
        })
        ...
        JsonCaseService.RunRobotTest(ServerAddr.value, ServerPort.value.toString(), RobotPrefix.value, Desc.value, FeiShuGuid.value, LoginTime.value, [], [],
            dir, {}, RoomOpTime.value, FeiShuNtf.value, DebugLevel.value, DebugLog.value, Concurrency.value, 0
        ).then(res => {
            flushLogCacheNow()
            message.info("用例运行结束")
        })
...
export const loadCases = async () => {
    ...
    // 刷新技能描述文案（路径由后端统一配置决定，与 .bytes 数据源独立）
    // await 等 excelSkillDescMap 加载完成，避免与下拉 render-label 渲染竞态导致 tooltip 间歇失效；
    // refreshSkillDescConfig 内部已 try-catch（失败时 map 置空、tooltip 退化为不显示），不会 reject，不会阻塞 loadCases
    await refreshSkillDescConfig()
