...
/** 确保步骤 uuid 稳定，并将 step.id 同步为当前列表序号（1..n） */
export function normalizeCaseSteps(steps?: (Step & DragAttr)[]) {
    steps?.forEach((step, index) => {
        ...
        step.assets?.forEach(asset => {
            const a = asset as Asset & DragAttr
            if (!a.uuid) {
...
/** @deprecated 使用 normalizeCaseSteps */
export function ensureCaseStepsUuids(caseOption?: TreeOption & ExtraCaseTreeOption) {
    normalizeCaseSteps(caseOption?.caseSteps)
}
