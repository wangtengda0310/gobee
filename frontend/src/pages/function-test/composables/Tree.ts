...
import {ExtraCaseTreeOption, DragAttr, dataRef, showCasesDesc} from "./use-case-data";
import {selectCaseByOption} from "./use-case-selection";
...
/**
 * 在 dataRef 中定位用例节点，并展开其父分类（供日志 Tab / 批量执行联动左侧树）。
 */
export function resolveCaseInTreeAndExpand(
    option: TreeOption & ExtraCaseTreeOption
): TreeOption & ExtraCaseTreeOption {
    for (const cate of dataRef) {
        const child = cate.children?.find(
            c => c.key === option.key || c.label === option.label
        )
        if (child) {
            if (cate.key != null) {
                const cateKey = String(cate.key)
                const expanded = expandedKeysRef.value ?? []
                if (!expanded.includes(cateKey)) {
                    expandedKeysRef.value = [...expanded, cateKey]
                }
            }
            return child as TreeOption & ExtraCaseTreeOption
        }
    }
    return option
}
