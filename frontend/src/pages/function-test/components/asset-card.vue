...
import {generateAiDesc} from "../composables/use-asset-ai-desc"
import {
  AssetEnum,
  assetSelectOption,
...
const updateAssetListToAssetMap = () => {
  console.log("prev: ", asset.value, assetList.value)
  if (asset.value)
    assetList2AssetMap(asset as Ref<Asset>, assetList, ackBool)
  console.log("before: ", asset.value, assetList.value)
}
...
/** 生成智能描述（代理到 composable） */
const aiDesc = () => {
  if (!step.value || !asset.value) return "请输入描述"
  return generateAiDesc(asset.value, assetList.value, step.value)
}

/** 应用智能描述到 desc 字段 */
const applyAiDesc = () => {
  if (asset.value)
    asset.value.desc = aiDesc()
}
</script>

<template>
  <div v-if="step && asset" class="assetCard">
    <!-- 第一行：资产类型选择 + 智能描述按钮 + 描述输入框 -->
    <div class="assetTypeRow">
      ...
      <n-tooltip trigger="hover" :style="{ maxWidth: '700px' }">
        <template #trigger>
          <n-button secondary type="success" @click="applyAiDesc(); updateNtf()">
            应用智能描述->
          </n-button>
        </template>
        {{ aiDesc() }}
      </n-tooltip>
      <n-input v-model:value="asset.desc" :placeholder="aiDesc()"
               @update:value="handleSectionUpdate"/>
    </div>
...
.assetTypeRow > :nth-child(1) {
  flex: 0 0 200px;
}

.assetTypeRow > :nth-child(2) {
  flex: 0 0 100px;
}

.assetTypeRow > :nth-child(3) {
  flex: 1 1 900px;
}
