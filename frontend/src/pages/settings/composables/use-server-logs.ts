...
// 前端就绪后请求后端回放历史日志：启动期日志（如 game InitExcel）在 app.Run 之前 Emit，
// 此时前端尚未监听 serverLog 事件会丢失；后端 ServerLogService 缓存了历史，收到此请求后回放。
Events.Emit('serverLogHistoryRequest')
