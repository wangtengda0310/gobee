...
| **应用更新** | **`UpdateService`** (`backend/pkg/settings/update/`) | 服务端 `https://itsnot.fun/rain-qa-func/latest.json` |
| **加载示例数据** | **`ExampleDataService`** (`backend/pkg/exampledata/`) | 释放 go:embed 示例到私有目录 + 配置 function-test(C 方案,独立包方便推翻删除,详见 [docs/Android-数据加载.md](../../../docs/Android-数据加载.md)) |
