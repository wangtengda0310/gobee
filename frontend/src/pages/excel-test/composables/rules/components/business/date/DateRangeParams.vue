<!--
  DateRangeParams - 日期范围检查规则参数组件
-->
<script setup lang="ts">
import {NInput} from "naive-ui"
import StandardBaseRow from '../../../base/StandardBaseRow.vue'

const props = defineProps<{ params: { [p: string]: string } }>()
</script>

<template>
  <div style="flex: 1 1 0; display: flex; flex-direction: column; justify-content: space-between; gap: 10px">
    <StandardBaseRow :params="params" :extra-defaults="{minDate: '', maxDate: ''}" />
    <div style="display: flex; gap: 10px; align-items: center">
      <div>最小日期:</div>
      <n-input
        style="flex: 1 1 0"
        placeholder="格式: 2006-01-02"
        :value="params['minDate']"
        @update:value="params['minDate'] = $event"
      />
      <div>最大日期:</div>
      <n-input
        style="flex: 1 1 0"
        placeholder="格式: 2006-01-02"
        :value="params['maxDate']"
        @update:value="params['maxDate'] = $event"
      />
    </div>
  </div>
</template>
