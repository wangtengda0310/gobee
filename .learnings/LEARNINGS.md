...
## [LRN-20260709-003] correction

**Logged**: 2026-07-09T23:30:00+08:00
**Priority**: medium
**Status**: resolved
**Area**: infra

### Summary
Windows PowerShell 5.1 以 GBK 解码 UTF-8 无 BOM 的 .ps1，含中文注释/字符串会乱码导致语法错误

### Details
为 Android 调试封装 `cdp_eval.ps1` 时，初版含中文注释和 `Write-Host` 中文字符串。Write 工具以 UTF-8(无 BOM) 落盘，Windows PowerShell 5.1 默认用 GBK 读取 .ps1，中文被解析成乱码(如"应用未运行"→"搴旂敤鏈繍"),触发 "Unexpected token )" / "string missing terminator" 等解析错误，整脚本无法执行。

根因：PowerShell 5.1 对无 BOM 的 .ps1 用系统 ANSI(中文 Windows = GBK/CP936)解码；只有带 UTF-8 BOM 才被识别为 UTF-8。PowerShell 7+ 默认 UTF-8 无此问题。

### Suggested Action
1. 给 Windows PowerShell 5.1 用的 .ps1 脚本，注释与字符串保持纯 ASCII，或以 UTF-8 BOM 保存。
2. 跨环境(5.1/7)复用的脚本统一纯 ASCII 最稳。

### Metadata
- Source: error
- Related Files: build/android/scripts/cdp_eval.ps1
- Tags: powershell, encoding, utf8, bom, gbk, script
- Pattern-Key: verify.ps1_ascii_or_bom
---
...
### Summary
rain-qa-func 后端在 Android 基本全面可用；调试用 CDP 远程调试(cdp_eval.ps1)，Go 业务日志在前端 serverLog 面板而非 logcat

### Details
2026-07-09 在 Small_Phone(x86_64)模拟器实证：后端 Go 代码在 Android 无崩溃级问题。可用(实证)：Service 调用链+文件IO(首页用例加载成功)、MCP server 监听 8765、proto-test 监听 TCP18000/HTTP20144、Go net.Listen/HTTP server、零 CGO + 平台代码 `//go:build` 隔离。降级(容错不崩)：VersionService(git)→N/A、OpenExcel(xdg-open)→失败、ExportClientConfig(cmd bat)→Windows 专属。非问题(排查勿误判)：cleartext HTTP 警告只影响 WebView 资源加载、不影响后端 Go 原生网络栈；Go 业务 log.Printf 不进 logcat，走前端 serverLog 面板(ServerLogService 用 os.Pipe → Event.Emit("serverLog"))。

调试方法(已沉淀到 docs/Android-运行时调试.md)：
- CDP(最强)：Wails Android 默认开 WebView 调试，`adb forward tcp:9223 localabstract:webview_devtools_remote_<pid>`，封装脚本 `build/android/scripts/cdp_eval.ps1 -Js "..."` 可执行任意前端 JS(读 DOM/点击导航/验证 Service)。
- logcat 看启动崩溃(grep Wails/AndroidRuntime/panic)；业务日志看前端面板。
- 端口验证：`adb shell cat /proc/net/tcp`，state 0A=LISTEN，端口字段十六进制。

### Suggested Action
1. 排查"Android 后端能否用"时，重心放在输入数据(配表/资源目录可达性)而非代码兼容性——已实证基本可用。
2. 调试 Android 用 `cdp_eval.ps1`(详见 docs/Android-运行时调试.md)，勿只在 logcat 找 Go 业务日志。

### Metadata
- Source: investigation
- Related Files: docs/Android-运行时调试.md, build/android/scripts/cdp_eval.ps1, cmd/rain-qa-func/wails.go
- Tags: android, cdp, debug, runtime, backend-availability, wails
- Pattern-Key: verify.android_cdp_debug
- See Also: LRN-20260709-001, LRN-20260709-002
---

## [LRN-20260710-001] knowledge_gap

**Logged**: 2026-07-10T01:25:00+08:00
**Priority**: high
**Status**: resolved
**Area**: backend

### Summary
Android c-shared(GOOS=android)进程 CWD=/ 且环境无 HOME/TMPDIR/XDG,Go 程序不能依赖 CWD 或 os.UserConfigDir/UserHomeDir 定位可写目录

### Details
rain-qa-func 的 appconfig 曾用 os.Getwd() 定位 .rain-qa-func.json,Android 模拟器实测:
- `readlink /proc/<pid>/cwd` → `/`(根目录,只读)
- `cat /proc/<pid>/environ` → 无 HOME/TMPDIR/XDG(仅 ANDROID_DATA 等)
故 `os.Getwd()`=/ → 配置写 `/.rain-qa-func.json` 失败;`os.UserConfigDir()/UserHomeDir()/UserCacheDir()` 均依赖 `$HOME`,在 Android 全部返回 error。

应用可写目录是私有目录 `/data/data/<package>/files`(应用 uid 可读写,profileInstalled 等普通文件写于此)。包名从 `/proc/self/cmdline` 首字段读取。

### Suggested Action
Android(GOOS=android)Go 代码写配置/缓存/数据时,不要用 CWD 或 `os.User*Dir()`;改用 `/data/data/<pkg>/files`(包名取自 `/proc/self/cmdline`),用 `runtime.GOOS` 分支保持桌面原逻辑。

### Metadata
- Source: investigation
- Related Files: backend/pkg/common/appconfig/appconfig.go, docs/Android-前端适配.md
- Tags: android, c-shared, cwd, userconfigdir, files-dir, golang
- Pattern-Key: verify.android_c_shared_cwd_root
- See Also: LRN-20260709-004
---

## [LRN-20260710-002] knowledge_gap
...
### Summary
rain-robot 的 .bytes 游戏数据版本必须与 go.mod rain-robot proto 定义版本严格匹配,否则 proto Unmarshal 报 wireType 错、game 加载失败

### Details
rain-qa-func 的 game 模块用 excel_manager_impl 加载 .bytes(protobuf 序列化游戏配表)。实测用本机 `D:\work\rain-robot\.../resources`(git 8ab59f7, 2026-05-11)的 .bytes,而 go.mod rain-robot = v0.0.0-20260527084927(2026-05-27)。版本不一致:05-11→05-27 间 SkillsTemplate proto 的 InitPros 字段 wireType 变,.bytes(旧 proto 序列化)被新 proto 反序列化报 `proto: wrong wireType = 0 for field InitPros`,load 失败,game IsInitialized=false。

修复:用与 go.mod rain-robot 同版本的 .bytes——go mod 缓存 `rain-robot@v0.0.0-20260527084927/.../resources`(182 .bytes),proto 匹配,加载成功(GetAllHeroCfg 返回 219 武将)。md5 验证 .bytes 未损坏,确认是版本不匹配而非传输问题。

桌面同样用本机 05-11 .bytes + go.mod 05-27 proto,理论上也加载失败(未实测确认)。

### Suggested Action
1. rain-robot .bytes 游戏数据版本必须与 go.mod rain-robot proto 版本对齐;换 rain-robot 版本时同步换 .bytes。
2. 正确 .bytes 来源:`go mod 缓存 rain-robot@<go.mod版本>/project/xcard/xcard_excel/resources`(与编译进二进制的 proto 同版本)。
3. game 加载失败(wiredType/IsInitialized=false)首先查 .bytes vs proto 版本匹配。

### Metadata
- Source: investigation
- Related Files: backend/pkg/common/game/excel.go, go.mod
- Tags: rain-robot, protobuf, version-skew, game-data, bytes, wiredType
- Pattern-Key: verify.rain_robot_bytes_proto_version_align
- See Also: LRN-20260710-001
---

### bindings generate .ts→.js 坑(2026-07-13)
跑 `wails3 generate bindings`(无 -tags android)删了 android .ts bindings 生桌面 .js → vue-tsc 类型丢(activity-wiki-check 'Basic' 报 TS2339);又跑 `-tags android -clean=true` 仍只生 .js。
根因:alpha2.117 generate 只产 .js,项目用 .ts(HEAD 161 .ts 0 .js 历史遗留)。
纠正:git checkout 恢复 .ts + 手写 exampledata .ts(仿 update updateservice.ts/models.ts/index.ts,$Call.ByID hash 2065048160 从 generate .js 读)+ 删所有 generate 产 .js。
教训:改全局 bindings 前 `git ls-files 'frontend/bindings/**/*.js'` 确认 HEAD 格式;generate -clean 破坏性,新服务手写 .ts 更稳。
违反:本可用 git ls-files 确认就跑 generate,属信息不足动手(CLAUDE.md "信息验证" + "掌握信息不足让用户澄清")。

### 用例树"测试类别[1]"双根因(2026-07-13,模拟器 CDP 实测定位)
现象:加载示例数据成功(10 用例释放 + 配置写盘),但战斗测试用例树显示默认"测试类别[1]"。
CDP 实测(nodes=1)定位双根因:
1. 竞态:Option.ts:16 GetConfig 模块级 async,Func.ts loadCases mount 时未 resolve → 用默认值 → InitExcel ../rain-robot 失败 + GetCategories("json") 空。serverLog InitExcel path=默认 证实。
2. loadCases 不自动:dataRef=createData() 默认,loadCases 只在 Menu.ts:25 手动"加载用例"按钮调,不在 mount 自动。
修复:Func.ts loadCases 开头 await GetConfig 刷新 + ExampleDataCard loadExample 后自动调 loadCases(去 reload)。
验证:模拟器 CDP 点加载示例 → nodes=10 + serverLog hasExample-data=true。
教训:CDP 读 serverLog(Go 日志面板)定位后端实际行为(InitExcel path/CheckUpdate),优于前端推理竞态。
### 下载 0% 不动根因:headApk 无超时(2026-07-13)
旧版 DownloadApk headApk 用 http.Head 默认无超时 → HEAD 卡则永久卡(emitProgress 在分块后才触发,headApk 阶段 0% 无信号)。
修复:headApk context 15s 超时 + DownloadApk 开始 emitProgress(0) + 分块 2MB+30s 超时+重试+断点续传。
模拟器验证:11→12 下载 .part 持续增长(8.4MB→40MB),headApk total 成功。
教训:HTTP 下载卡死优先查超时(net/http 默认 Client 无 Timeout);下载进度初始 emitProgress(0) 让用户知道启动。
