...
func NewJsonCaseService(app *application.App) *JsonCaseService {
...
type CustomHero struct {
	...
	Color         int32               `json:"color"`         // @json-critical 阵营颜色
...
func (a *JsonCaseService) RunRobotTest(
	...
	xcard_client.RobotsYanWuQAFunctionCase(ip, port, prefix, desc, feishuGuid, loginTime, filterCasesOption, filterCaseNameOption,
		dir, filesData, concurrency, opTimeMs, actualFeiShuNtf, debugLevel, 10*time.Minute,
				...
				for {
					...
					case <-ticker.C:
						...
						case log := <-wails3LogChan:
							a.app.Event.Emit("robotLog", log, seq.Load())
							seq.Add(1)
