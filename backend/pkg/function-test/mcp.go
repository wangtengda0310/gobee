...
func RegisterJsonCaseTools(s *mcpgo.Server, svc *JsonCaseService) {
	...
	s.AddTool(&mcpgo.Tool{
		...
		// 清日志缓存，确保 validate 反映本次执行（非上次残留）
		svc.ClearTestLogs()
		...
		err := svc.RunRobotTest(
			...
			args.Concurrency,
			0, // maxTimeoutPerCase: 0=默认 10 分钟（MCP 不暴露超时）
		)
		...
		// 防 filterCases/filterCaseNames 未命中空跑（RunRobotTest 恒返 nil，services.go:230）
		if err := validateFilterCaseNamesRan(svc.GetTestLogs(""), args.FilterCaseNames); err != nil {
			return mcp.ErrorResultFromError(err), nil
		}
	...
	// update_case - 按 case 名原地更新一条用例（保持顺序）
	s.AddTool(&mcpgo.Tool{
		Name:        "update_case",
		Description: "按 case 名原地更新一条用例（保持数组顺序不变）。推荐流程：先用 get_case_by_name 读出用例，修改后整体传回 newCase。用例不存在则失败。",
		InputSchema: map[string]any{
			"type": "object",
			"properties": map[string]any{
				"filePath": map[string]any{
				...
				"caseName": map[string]any{
					"type":        "string",
					"description": "要更新的用例名（Case 字段，用于定位原用例）",
				},
				"newCase": map[string]any{
					"type":        "object",
					"description": "新的用例完整内容（QAFuncCase 结构：case/desc/manager/initYanWu/steps）",
				},
			},
			"required": []string{"filePath", "caseName", "newCase"},
		},
	}, func(ctx context.Context, req *mcpgo.CallToolRequest) (*mcpgo.CallToolResult, error) {
		var args struct {
			FilePath string     `json:"filePath"`
			CaseName string     `json:"caseName"`
			NewCase  QAFuncCase `json:"newCase"`
		}
		if err := json.Unmarshal(req.Params.Arguments, &args); err != nil {
			return mcp.ErrorResult("解析参数失败: " + err.Error()), nil
		}

		if err := svc.UpdateCase(args.FilePath, args.CaseName, args.NewCase); err != nil {
			return mcp.ErrorResultFromError(err), nil
		}

		return mcp.TextResult("用例 " + args.CaseName + " 已更新"), nil
	})
}
...
// buildFightTestResult 根据 runTestFunc 的返回构建战斗测试结果。
// 空 logs（filterCases/caseName 未命中、0 用例执行的空跑）视为失败——
// 避免 run_fight_test 空跑误报 success=true（详见 memory l6-run-fight-test-fake-pass）。
func buildFightTestResult(logs map[string][]LogEntry, err error) FightTestResult {
	result := FightTestResult{
		Success: err == nil && len(logs) > 0,
		Message: "战斗测试完成",
		Logs:    logs,
	}
	switch {
	...
	case len(logs) == 0:
		result.Message = "未执行任何用例（filterCases/caseName 未匹配任何用例，可能路径错误或用例不存在）"
	}
	return result
}

// validateFilterCaseNamesRan 核对 run_robot_test 执行后的日志是否覆盖 filterCaseNames。
// 防 filterCaseNames 未命中空跑（RunRobotTest 恒返 nil，services.go:230）。
// handler 调 RunRobotTest 后用它：传 GetTestLogs("") 与 filterCaseNames。
func validateFilterCaseNamesRan(allLogs map[string][]LogEntry, filterCaseNames []string) error {
	if len(allLogs) == 0 {
		return fmt.Errorf("未执行任何用例（filterCases/filterCaseNames 未匹配，可能路径错误或用例不存在）")
	}
	if len(filterCaseNames) == 0 {
		return nil // 未指定 filterCaseNames，不核对
	}
	var missed []string
	for _, name := range filterCaseNames {
		if _, ok := allLogs[name]; !ok {
			missed = append(missed, name)
		}
	}
	if len(missed) > 0 {
		return fmt.Errorf("以下用例未执行（filterCaseNames 未匹配）: %v", missed)
	}
	return nil
}
...
func RegisterFightCaseTools(s *mcpgo.Server, fightCasesDir string, runTestFunc func(heroID int, caseName string, filterCases []string) (map[string][]LogEntry, error), feishuNotifyConfigSvc feishu.FeishuNotifier) {
	...
	s.AddTool(&mcpgo.Tool{
		...
		result := buildFightTestResult(logs, err)
	...
	// reverse_translate_case - 语义回译用例（三维度反推：initYanWu/steps/assets）
	// 把 JSON 用例机械地反推成中文描述，用于 L4/L5 与飞书文档三列客观比对。
	s.AddTool(&mcpgo.Tool{
		Name:        "reverse_translate_case",
		Description: "语义回译：把指定测试用例按固定模板反推成中文描述（前置条件/步骤/预期结果三维度），用于与飞书文档客观比对。assets 维度精确复刻前端 generateAiDesc。",
		InputSchema: map[string]any{
			"type": "object",
			"properties": map[string]any{
				"filePath": map[string]any{
		...
		// 用 JsonCaseService 读取用例（app 传 nil，GetTestCase/ReadJSONFileOrDir 不依赖 app）
		jsonSvc := NewJsonCaseService(nil)
		qaFuncCase, err := jsonSvc.GetTestCase(args.FilePath, args.CaseName)
		if err != nil {
			return mcp.ErrorResultFromError(err), nil
		}
		if qaFuncCase == nil {
		...
		result := ReverseTranslateCase(*qaFuncCase, newNameResolver(gameExcel))
...
func runFightTestAsync(taskID string, heroID int, caseName string, filterCases []string, runTestFunc func(heroID int, caseName string, filterCases []string) (map[string][]LogEntry, error)) {
	...
	result := buildFightTestResult(logs, err) // 复用判定：空跑（logs 空）也判失败，与 run_fight_test 一致

	// 更新最终状态
	globalTaskManager.updateTask(taskID, func(t *TestProgress) {
		if !result.Success {
			t.Status = TaskStatusFailed
			t.ErrorMessage = result.Message
			return
		}
