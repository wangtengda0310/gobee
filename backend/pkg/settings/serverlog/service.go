...
// maxHistory 历史日志缓存上限。
// 启动期日志（如 game InitExcel）在 app.Run 之前 Emit，前端尚未监听 serverLog 事件会丢失；
// forwardLogs 同时缓存历史，前端就绪后通过 serverLogHistoryRequest 事件请求回放。
...
type ServerLogService struct {
	app       *application.App
	writer    *PipeWriter
	history   []map[string]any // 历史日志缓存（前端连上前 Emit 会丢失，连上后回放）
...
func (s *ServerLogService) InitWithApp(app *application.App) {
	...
	// 前端连上后（EventsEmit "serverLogHistoryRequest"）回放历史日志：
	// 启动期日志（如 game InitExcel）在 app.Run 之前 Emit，前端尚未监听会丢失，
	// 故 forwardLogs 同时缓存历史，前端就绪后请求回放。
	if s.app != nil {
		s.app.Event.On("serverLogHistoryRequest", func(_ *application.CustomEvent) {
			s.historyMu.Lock()
			snapshot := make([]map[string]any, len(s.history))
			copy(snapshot, s.history)
			s.historyMu.Unlock()
			for _, m := range snapshot {
				s.app.Event.Emit("serverLog", m)
			}
		})
	}
...
// 同时缓存到 history，供前端连上后回放（启动期日志在前端监听前 Emit 会丢失）
...
func (s *ServerLogService) forwardLogs() {
	for entry := range s.writer.Entries() {
		m := entry.ToMap()
		// 缓存到历史（前端连上前 Emit 会丢失，连上后通过 serverLogHistoryRequest 回放）
		s.historyMu.Lock()
...
func (s *ServerLogService) emitManual(level string, msg string) {
	...
	// 缓存到历史（与 forwardLogs 一致，确保手动日志也可回放）
	s.historyMu.Lock()
	s.history = append(s.history, m)
	if len(s.history) > maxHistory {
		s.history = s.history[len(s.history)-maxHistory:]
