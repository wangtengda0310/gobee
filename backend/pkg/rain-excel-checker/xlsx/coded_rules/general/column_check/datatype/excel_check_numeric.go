...
// NumericCheckRule 数值检查
type NumericCheckRule struct{}

func (c *NumericCheckRule) Check(sheetName string, cols [][]string, colIdx, startRowIdx int, params map[string]string, sheetMap map[string]*excelize.File) []*json_rule.CellError {
	...
	for i, s := range myColData {
		...
		if err != nil {
			res = append(res, &json_rule.CellError{
				...
				Reason:   fmt.Sprintf("不是有效的数值: %s", s),
			})
		}
	}
	return res
