...
// RemovedRowNotifyRule 删除行通知规则
// 检测 Excel 表中被删除的数据行，生成变更通知
type RemovedRowNotifyRule struct{}

func (c *RemovedRowNotifyRule) Meta() *json_rule.TableRuleMeta {
	return &json_rule.TableRuleMeta{
		Type:         json_rule.REMOVED_ROW_NOTIFY,
		DisplayName:  "删除行通知",
		Description:  "检测Excel表中被删除的数据行，生成变更通知。仅在有删除行时产生通知。",
		TargetSheets: []string{},
...
func (c *RemovedRowNotifyRule) Check(param json_rule.CheckParam) *json_rule.TableCheckResult {
	result := &json_rule.TableCheckResult{
		Ok:          true,
		RuleType:    json_rule.REMOVED_ROW_NOTIFY,
		DisplayName: "删除行通知",
	...
	if entry.DiffResult == nil || len(entry.DiffResult.RemovedRows) == 0 {
		return result
	}
	...
	notification.WriteString(formatRemovedRowsReason(entry.DiffResult.RemovedRows, ctx))
	result.Reason = notification.String()

	for _, row := range entry.DiffResult.RemovedRows {
		result.ErrCells = append(result.ErrCells, &json_rule.CellError{
			Index:    row.RowIndex,
			ExcelRow: row.RowIndex + 1,
			Reason:   fmt.Sprintf("删除行: %s(id=%s)", row.RowName, row.RowId),
