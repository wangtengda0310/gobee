...
// RemovedColNotifyRule 删除列通知规则
// 检测 Excel 表中被删除的列，生成变更通知
type RemovedColNotifyRule struct{}

func (c *RemovedColNotifyRule) Meta() *json_rule.TableRuleMeta {
	return &json_rule.TableRuleMeta{
		Type:         json_rule.REMOVED_COL_NOTIFY,
		DisplayName:  "删除列通知",
		Description:  "检测Excel表中被删除的列，生成变更通知。仅在有删除列时产生通知。",
		TargetSheets: []string{},
...
func (c *RemovedColNotifyRule) Check(param json_rule.CheckParam) *json_rule.TableCheckResult {
	result := &json_rule.TableCheckResult{
		Ok:          true,
		RuleType:    json_rule.REMOVED_COL_NOTIFY,
		DisplayName: "删除列通知",
	...
	if entry.DiffResult == nil || len(entry.DiffResult.RemovedCols) == 0 {
		return result
	}
	...
	notification.WriteString(formatRemovedColsReason(entry.DiffResult.RemovedCols, ctx))
	result.Reason = notification.String()

	for _, colName := range entry.DiffResult.RemovedCols {
		result.ErrCells = append(result.ErrCells, &json_rule.CellError{
			Index:  -1,
			Reason: fmt.Sprintf("删除列: %s", colName),
			Detail: &json_rule.ColumnChangeDetail{
				ChangeType: "colRemoved",
				ColName:    colName,
			},
		})
	}
