...
func main() {
	...
	ecs.EventBus().Subscribe(EventKill, func(e Event) {
		fmt.Printf("[事件] Entity %d 被Kill, 原因: %v\n", e.Entity, e.Data)
	})
	// 高级事件系统示例
	ecs.AdvancedBus().Subscribe(EventGrowth, func(e Event) {
		fmt.Printf("[高级事件] Entity %d 成长: %v\n", e.Entity, e.Data)
	})
	...
	fmt.Printf("Entity %d 初始: Attack+Velocity\n", id)
	ecs.AddComponent(id, Shape, ShapeData{Name: "Square"})
	fmt.Printf("Entity %d 增加Shape\n", id)
	ecs.RemoveComponent(id, Attack)
	fmt.Printf("Entity %d 移除Attack\n", id)
	...
	fmt.Printf("并行系统处理100万entity耗时: %v\n", time.Since(start))

	// 复杂事件链测试
	ecs.AdvancedBus().Subscribe(EventHide, func(e Event) {
		fmt.Printf("[链式事件] Entity %d 被隐藏，自动触发成长\n", e.Entity)
	...
	fmt.Println("ECS系统复杂事件与system测试完成", time.Since(now))

	// 生命周期钩子测试
	RegisterOnCreate(func(e *Entity) { fmt.Printf("[生命周期] 创建Entity %d\n", e.ID) })
	RegisterOnDestroy(func(e *Entity) { fmt.Printf("[生命周期] 销毁Entity %d\n", e.ID) })

	// 标签分组批量操作
	for i := 0; i < 10; i++ {
		_ = ecs.CreateEntityWithTags(map[ComponentType]interface{}{Attack: AttackData{Value: i}}, "enemy")
	}
	fmt.Printf("enemy标签下entity数量: %d\n", len(ecs.EntitiesWithTag("enemy")))

	// 组件组合
	combo := CompositeComponent{Components: []ComponentType{Attack, Velocity}}
	fmt.Printf("组合组件bitmask: %d\n", combo.Type())
	...
	fmt.Printf("System热插拔后第一个system: %s\n", sm.systems[0].Name())

	// 快照/回滚
	snap := ecs.Snapshot()
	id = ecs.CreateEntityWithTags(map[ComponentType]interface{}{Attack: AttackData{Value: 99}}, "hero")
	fmt.Printf("创建新entity: %d\n", id)
	ecs.Restore(snap)
	fmt.Printf("回滚后hero标签下entity数量: %d\n", len(ecs.EntitiesWithTag("hero")))
	...
	peb.Subscribe(EventKill, EventSubscription{
		...
		Handler:  func(e Event) { fmt.Printf("[高优先级偶数Kill] %d\n", e.Entity) },
	})
	peb.Subscribe(EventKill, EventSubscription{
		Priority: 1,
		Handler:  func(e Event) { fmt.Printf("[低优先级Kill] %d\n", e.Entity) },
	})
	for i := 0; i < 5; i++ {
	...
	log := ebus.SaveLog()
	fmt.Println("事件日志保存，重放:")
	ebus.Replay(log)

	// 性能分析
	ecs.WriteProfile("ecs.prof")
	fmt.Println("已写入heap profile: ecs.prof")

	// 脚本注册
	RegisterScriptSystem("PrintHello", func(args ...interface{}) { fmt.Println("[脚本System] Hello", args) })
	...
	if err == nil {
		...
		fmt.Println("未找到配置文件，跳过配置加载测试")
	}

	// Mock
	mock := MockEntity(999, Attack|Velocity, "mock")
	fmt.Printf("Mock entity: %+v\n", mock)
