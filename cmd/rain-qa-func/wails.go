...
import (
	...
	"git.devcloud.ztgame.com/v-tangfangda/rain-qa-func/backend/pkg/settings/update"
	exampledata "git.devcloud.ztgame.com/v-tangfangda/rain-qa-func/backend/pkg/exampledata"
...
func runWails() {
	...
	// 注：gameExcelSvc.InitExcel 延后到 serverLog 启动后执行（见下文），以便 InitExcel/load 的日志
	// 能被 serverLog 捕获（否则在 serverLog 拦截 stdout 前发出会被丢弃，Android 排查困难）
	...
	app.RegisterService(application.NewService(functiontest.NewJsonCaseService(app)))
	...
	// Android APK 自动更新服务(CheckUpdate 对比服务端 versionCode + DownloadApk 下载校验;
	// 前端调 Java WailsJSBridge.installApk 触发系统安装器。详见 docs/Android-自动更新.md)
	updateService := update.New()
	updateService.InitWithApp(app)
	app.RegisterService(application.NewService(updateService))

	// 内置示例数据服务(Android 数据供给 C 方案:go:embed resources + fight_cases,
	// "加载示例"释放到私有目录 + 配置 function-test 指向。独立包,推翻时删包 + 去本注册)
	app.RegisterService(application.NewService(exampledata.New()))

	// 系统干净:启动清已安装的旧 APK(保留 .part 续传),避免 updates/ 长期累积
	// (方案 🅱️:安装失败可重试因 APK 未删;装成功后下次启动清)
	update.CleanInstalledApks()

	// game Excel 预初始化（延后到 serverLog 之后执行，使 InitExcel/load 日志可被捕获，便于排查资源加载失败）。
	// 仅移动执行时机，逻辑不变；前端在 app.Run 之后才会调用 game，此时已完成初始化。
